...
- Keep the git-install path free of devDependency binaries. `prepare` runs `build` when the package is installed from a git ref, and pi installs extensions with `npm install --omit=dev`, so appending `tsc --emitDeclarationOnly` to `build` broke every git install with `sh: 1: tsc: not found` (exit 127) *after* the esbuild bundle had already succeeded. `build` is now the bundle alone, declarations moved to `build:types`, and `prepack` runs both so a published tarball still ships `dist/index.d.ts`. Only `dist/index.js` is needed to load the extension.
